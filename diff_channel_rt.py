...
feature_ids1 = ds1.variables[ 'feature_id' ]
feature_ids2 = ds2.variables[ 'feature_id' ]
...
# check feature ids
#
if not np.array_equal( feature_ids1[:], feature_ids2[:] ):
    print( 'NetCDF files have different feature_ids!' )
    sys.exit(1)
   
diff = np.subtract( var1, var2 )

for d, id in zip( diff, feature_ids1[:] ):
   if abs( d ) >= tol:
     print( id, " : ", d )
